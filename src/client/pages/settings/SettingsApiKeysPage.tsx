...
import type { ApiKeyResponse, RotateResponse } from './types';
        ...
        <div className="mt-4 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              readOnly
              value={newKey}
              className="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
            />
            <button
              onClick={() => copyKey(newKey)}
              className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-600 hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
            >
              {copied ? '✓' : '📋'}
            </button>
          </div>
          <button onClick={() => setNewKey(null)} className="mt-2 text-sm text-gray-500 hover:text-gray-400">
            I've saved this key
          </button>
        </div>
      )}
